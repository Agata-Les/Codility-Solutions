...
Write a function that, given an array A consisting of N integers, returns index of any element of array A 
...
int FindDominator(vector<int> &A)
{
    ...
    int dominator = B.at(0);
    unsigned int counter = 1;
    bool success = false;
    
    for (unsigned int i = 1; i < B.size(); ++i)
    {
        if (dominator == B.at(i)) ++counter;
        else
        {
            dominator = B.at(i);
            counter = 1;
