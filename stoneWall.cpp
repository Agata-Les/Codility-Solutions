...
// of N positive integers. H[I] is the height of the wall from I to I+1 meters to the right of its left end. In particular, 
...
int solution(std::vector<int> &H)
{
    ...
    for(unsigned int i = 0; i < H.size(); ++i)
    {
        if(!storage.empty())
        {
            const int previousBlock { storage.back() };
            
            if(H[i] > previousBlock) 
            {
                storage.push_back(H[i]);
                ++counter;
            }
            
            else if(H[i] < previousBlock)
            {
                storage.pop_back();
                --i;
            }
