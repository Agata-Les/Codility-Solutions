...
int solution(vector<int> &A, vector<int> &B) {
    std::vector<std::pair<int, int>> survivors { std::make_pair(A[0],B[0]) };
    
    for(unsigned int i = 1; i < B.size(); ++i)
    {
        if(B[i] == 0 && survivors.back().second == 1)
        { 
            if(A[i] > survivors.back().first)
            {
                survivors.pop_back();
                if(!survivors.empty()) --i;
                else survivors.push_back( std::make_pair(A[i], B[i]) );
            }   
        }
        else survivors.push_back( std::make_pair(A[i], B[i]) );
    }
